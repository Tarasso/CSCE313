...
MQRequestChannel::MQRequestChannel(const string _name, const Side _side, int _bufferCapacity) : RequestChannel (_name, _side) {
...
int MQRequestChannel::open_ipc(string _pipe_name, int mode){
	...
	attrib.mq_flags = 0;
	attrib.mq_maxmsg = 1;
	attrib.mq_msgsize = bufferCapacity;
	attrib.mq_curmsgs = 0;

	int fd = (int) mq_open(_pipe_name.c_str(), mode, 0600, &attrib);
	if (fd < 0){
